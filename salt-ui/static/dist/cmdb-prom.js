$(document).ready( function () {
    var table = $('#datatables').DataTable({
                ...
                "retrieve": true, //检索一个已存在的Datatables实例
                    "columns": [
                        {"mdata": "fun_categ_name"}, 
                        {"mdata": "soft_group_name"},
                        {"mdata": "group_mobile"},  
                        {"mdata": "server_group_id"}, 
                        {"mdata": "Modify"},  
                    ], 
                     "columnDefs":[ 
                          { 
                         ...
                         { 
                            ...
                            "render":function(data,type,row){ 
                                return "<button data-names='"+row[0]+"' id='del-btn' type='button' class='btn btn-danger btn-sm'>删除</button>&nbsp;" +
                                        "<button data-serverip='"+row[0]+"' data-sname='"+row[1]+"' data-prom='"+row[3]+"' data-env='"+row[4]+"' data-status='"+row[5]+"' id='edit-btns' type='button' class='btn btn-success btn-sm'>编辑</button>&nbsp;" +
            ...
            $("#submit-update-btn").on('click', function (data) { 
                ...
                $.ajax({
                    type: "post",
                    url: "/cmdb/update?action_type=server_list",
             ...
             //submit add node
            $("#submit-addnode-btn").on('click', function (data) { 
                var serverip = $("#front-data-serverips").val()
                var servername = $("#front-data-server-names").val()
                var prom = $("#front-data-proms").val()
                var env = $("#front-data-envs").val()
                var status = $("#front-data-statuss").val()
                ...
                swal({   title: "请确认",
                    ...
                    confirmButtonText: "确认, 添加!",
                    closeOnConfirm: false },
                    function() {
                     $.ajax({
                    type: "post",
                    url: "/cmdb/add?action_type=server_list",
                    data: {"serverip":serverip,"servername":servername,"prom":prom,"env":env,"status":status},
                    success: function(datass) {
                        if (datass==0) {
                            //$('#update-modal').modal('hide')
                                    table.draw(false); // redrawing datatable 
                                    //alert("ok");
                                    swal("成功!", "文件已添加.", "success");
                                }
                        else if(datass== 'err') {
                            //$('#update-modal').modal('hide')
                                    table.draw(false); // redrawing datatable 
                                    //alert("ok");
                                    swal("错误!", "IP已存在.", "warning");
                                }
                        else {
                             swal("未知!", "错误.", "warning");
                        }
            ...
            $("#addnodes").on('click', function (data) { 
                ...
                var servername = $(this).attr("data-sname")
                var prom = $(this).attr("data-prom")
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        strs += '<select class="form-control" name="front-data-prom" id="front-data-proms">'
                        ...
                        str += '<select class="form-control" name="front-data-env" id="front-data-envs" >'
                        ...
                        str += '<select class="form-control" name="front-data-status" id="front-data-statuss" >'
            ...
            ////update
            $("#datatables tbody").on('click','button#edit-btns', function (data) { 
                var serverip = $(this).attr("data-serverip")
                var serverName = $(this).attr("data-sname")
                var prom = $(this).attr("data-prom")
                var env = $(this).attr("data-env")
